...
func TestServiceHandleInboundCoversMinimumLoopAcceptance(t *testing.T) {
	...
	assert.Equal(t, "tenant:tenant-a:app:app-wecom:channel:wecom:dm:user-shared", wecomResult.SessionID)
	assert.Equal(t, "tenant:tenant-b:app:app-telegram:channel:telegram:dm:user-shared", telegramResult.SessionID)
	assert.Equal(t, "tenant:tenant-b:app:app-wecom:channel:wecom:dm:user-shared", tenantBWeComResult.SessionID)
	assert.Equal(t, "tenant:tenant-a:app:app-wecom:channel:wecom:group:room-1", groupResult.SessionID)
